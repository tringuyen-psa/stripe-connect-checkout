...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("🏪 Using Connect Account:", STRIPE_CONNECT_ACCOUNT_ID_TEST);

    // Sử dụng Connect Stripe instance để retrieve session
    if (STRIPE_CONNECT_ACCOUNT_ID_TEST) {
      const connectStripe = new Stripe(process.env.STRIPE_SECRET_KEY_TEST!, {
        apiVersion: "2025-09-30.clover",
        typescript: true,
        stripeAccount: STRIPE_CONNECT_ACCOUNT_ID_TEST,
      });

      const session = await connectStripe.checkout.sessions.retrieve(
        sessionId,
        {
      ...
      console.log(
        "✅ Successfully retrieved checkout session from Connect account"
      );
